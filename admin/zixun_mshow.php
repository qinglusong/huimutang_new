...
if ($rec == 'default') {
    ...
    $smarty->display('zixun_mshow.htm');
} 
...
elseif ($rec == 'insert') {
    ...
    //if (empty($_POST['show_name'])) $dou->dou_msg($_LANG['show_name'] . $_LANG['is_empty']);
        if (empty($_POST['show_style'])) $dou->dou_msg('请选择幻灯板式');
    // 文件上传盒子
    ...
    $firewall->check_token($_POST['token']);
     ...
     $sql = "INSERT INTO " . $dou->table('show') . " (id, show_name, contents, slogan, style_banner, show_link, show_img, type, sort)" . " VALUES (NULL, '$_POST[show_name]', '$_POST[show_contents]', '$_POST[show_slogan]', '$_POST[show_style]', '$_POST[show_link]', '$show_img', '$systems', '$_POST[sort]')";
    $dou->query($sql);
    
    $dou->create_admin_log($_LANG['show_add'] . ': ' . $_POST[show_name]);
    $dou->dou_msg($_LANG['show_add_succes'], 'zixun_mshow.php');
} 
...
elseif ($rec == 'update') {
    ...
    $firewall->check_token($_POST['token']);
     ...
     $sql = "update " . $dou->table('show') . " SET show_name='$_POST[show_name]'" . $show_img . " ,contents = '$_POST[show_contents]',slogan = '$_POST[show_slogan]',style_banner = '$_POST[show_style]',show_link = '$_POST[show_link]', sort = '$_POST[sort]' WHERE id = '$_POST[id]'";
    $dou->query($sql);
    
    $dou->create_admin_log($_LANG['show_edit'] . ': ' . $_POST[show_name]);
    $dou->dou_msg($_LANG['show_edit_succes'], 'zixun_mshow.php');
}
...
elseif ($rec == 'del') {
    // 验证并获取合法的ID
    $id = $check->is_number($_REQUEST['id']) ? $_REQUEST['id'] : $dou->dou_msg($_LANG['illegal'], 'zixun_mshow.php');
    ...
    if (isset($_POST['confirm']) ? $_POST['confirm'] : '') {
        ...
        $dou->delete($dou->table('show'), "id = '$id'", 'zixun_mshow.php');
    } else {
        $_LANG['del_check'] = preg_replace('/d%/Ums', $show_name, $_LANG['del_check']);
        $dou->dou_msg($_LANG['del_check'], 'zixun_mshow.php', '', '30', "zixun_mshow.php?rec=del&id=$id");
    }
}
