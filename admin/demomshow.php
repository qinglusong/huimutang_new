...
$smarty->assign('cur', 'demomshow');
$smarty->assign('ur_here', $_LANG['demomshow']);
$systems = 'demom';
...
elseif ($rec == 'insert') {
    ...
    $dou->dou_msg($_LANG['show_add_succes'], 'demomshow.php');
} 
...
elseif ($rec == 'update') {
    ...
    $dou->dou_msg($_LANG['show_edit_succes'], 'demomshow.php');
}
...
elseif ($rec == 'del') {
    // 验证并获取合法的ID
    $id = $check->is_number($_REQUEST['id']) ? $_REQUEST['id'] : $dou->dou_msg($_LANG['illegal'], 'demomshow.php');
    ...
    if (isset($_POST['confirm']) ? $_POST['confirm'] : '') {
        ...
        $dou->delete($dou->table('show'), "id = '$id'", 'demomshow.php');
    } else {
        $_LANG['del_check'] = preg_replace('/d%/Ums', $show_name, $_LANG['del_check']);
        $dou->dou_msg($_LANG['del_check'], 'demomshow.php', '', '30', "demomshow.php?rec=del&id=$id");
    }
}
