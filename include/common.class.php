...
class Common extends DbMysql {
     ...
     * +----------------------------------------------------------
     * $table 数据表名
     * $parent_id 默认获取二级导航 只有两个 id=1 or 2
     * $level 无限极分类层次
     * $current_id 当前页面栏目ID
     * $category_nolevel 储存分类信息的数组
     * $mark 无限极分类标记
     * +----------------------------------------------------------
     */
    function get_yiji_category($table) {
        $data = $this->fetch_array_all($this->table($table), 'sort ASC, cat_id ASC');
        foreach ((array) $data as $value) {
    ...
    function get_erji_category($table,$erji_id = 'all') {
        $data = $this->fetch_array_all($this->table($table), 'sort ASC, cat_id ASC');
        foreach ((array) $data as $value) {
            if($erji_id == 'all'){
                if ($value['parent_id'] == '1' || $value['parent_id'] == '2' ) {
                    $value['url'] = $this->rewrite_url($table, $value['cat_id']);
                    $category_nolevel[] = $value;
                }
            }else{
                if ($value['parent_id'] == $erji_id) {
                    $value['url'] = $this->rewrite_url($table, $value['cat_id']);
                    $category_nolevel[] = $value;
                }
            }
    ...
    function get_news_list() {
        $list = array ();
        $sql = "SELECT id FROM " . $this->table('article')." order by id asc ";
        $res = $this->query($sql);
        return $res;
    }
